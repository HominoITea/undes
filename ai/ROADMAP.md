...
| P1 | `shared-enabler` | Pipeline Cost & Efficiency Optimization | in-progress | Anthropic prompt caching is landed, complexity-based routing is landed in bounded form (`trivial` / `standard` / `complex` + `--full-panel` override), conditional DA skip is landed for diagnostic-no-fetchable and clean patch-safe high-approval cases, conditional tester skip for diagnostic results is landed, and rerun skip-enhance is landed via manual flag plus same-prompt preprocess reuse. Live validation on `plta-document-flow` (2026-03-21) partially confirmed the stack in a real rerun: preprocess reuse activated and routing classified the task as `standard`, but the run hit a reviewer Google timeout before final metrics were written. Next: repeat live validation on a stable-provider rerun path before adding more knobs. | `ai/design/features/PIPELINE_COST_OPTIMIZATION.md` |
| P1 | `shared-enabler` | Round Orchestration Rationalization | phase-1-landed | Phase 1 landed: early seam expansion trigger, max 2 approval rounds, no-material-progress stop, DA skip gate, tester patch-validation gate, telemetry collecting. Phase 2 gated by critiqueSeamOverlap >70% over 10+ runs. | `ai/design/features/ROUND_ORCHESTRATION_RATIONALIZATION.md` |
