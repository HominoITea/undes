...
| P1 | `shared-enabler` | Pipeline Cost & Efficiency Optimization | in-progress | Anthropic prompt caching is landed, complexity-based routing is landed in bounded form (`trivial` / `standard` / `complex` + `--full-panel` override), conditional DA skip is landed for diagnostic-no-fetchable and clean patch-safe high-approval cases, conditional tester skip for diagnostic results is landed, and rerun skip-enhance is landed via manual flag plus same-prompt preprocess reuse. Next: verify the full savings stack on live reruns rather than adding more knobs. | `ai/design/features/PIPELINE_COST_OPTIMIZATION.md` |
