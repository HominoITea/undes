...
function buildSameLanguageInstruction(options = {}) {
  ...
  if (jsonMode) {
    return 'The original user prompt above is authoritative for language. Use the same natural language for all free-text string values unless the user explicitly asks for another language. Keep prose dry and direct. Do not switch explanatory sentences, section labels, or service phrases into English on your own. English is allowed only for exact code identifiers, API names, required schema keys, or short technical terms that would become less clear if translated. Keep JSON keys, enum values, tables, and required markers exactly as specified.';
  }
  return 'The original user prompt above is authoritative for language. Respond in the same natural language unless the user explicitly asks for another language. Keep the prose dry and direct. Do not switch headings, explanations, transition phrases, or service wording into English on your own. English is allowed only for exact code identifiers, API names, or short technical terms when they are clearer than a translation.';
}
...
function buildGroundedFixesOutputContract() {
  return [
    ...
    'Each `Evidence:` line must contain only one exact file anchor, such as `src/File.ts`, `src/File.ts:123`, or `src/File.ts:120-140`.',
    'Do not append parenthetical explanations, code expressions, helper names, or prose on the same `Evidence:` line.',
    'If you need to explain why the anchor matters, do it in the next sentence or bullet, not inside the `Evidence:` line.',
    ...
    'If evidence proves only a scoped or partial patch, say that explicitly inside `Grounded Fixes` and do not present it as the full end-to-end fix.',
    'Do not claim an edge case, fallback branch, or special-case path is fully fixed unless that exact path was explicitly read or directly proven in this run.',
  ];
}
...
function buildProposalContent(promptText, discussionSoFar, roleLabel) {
  ...
  return [
    ...
    '\nUse this compact structure in order: `## Diagnosis`, `## Grounded Patch`, `## Risks / Assumptions`.',
    '\nKeep `## Diagnosis` to at most 3 bullets or short paragraphs total.',
    '\nKeep `## Grounded Patch` to at most 3 patch items. Merge duplicates instead of repeating similar fixes.',
    '\nKeep `## Risks / Assumptions` to at most 3 bullets. Omit the section entirely if there is nothing material to add.',
    ...
    '\nKeep each `Evidence:` line anchor-only: one exact path or line range, with no parenthetical code expressions or prose on that same line.',
...
`;
  ...
  return [
    ...
    ' Limit the response to the 3 most important table rows and at most 3 bullets in `My additions/objections`.',
    ' Keep each table comment to one short sentence whenever possible.',
...
function buildConsensusContent(promptText, discussionSoFar, roleLabel) {
  ...
  return [
    ...
    ' If the draft fixes only the directly proven branch or local defect, describe it as a scoped patch instead of a complete end-to-end fix.',
    ' Keep every section compact: no more than 5 bullets/items per section unless concrete code blocks require more space.',
    ' Prefer one short bullet per grounded claim instead of long prose paragraphs.',
    '\n\n# OUTPUT CONTRACT\n',
...
function buildConsensusReviewContent(promptText, discussionSoFar, consensusText, roleLabel) {
  ...
  return [
    ...
    ' - `Grounded Fixes` presents a scoped or branch-local patch as a complete fix even though a fallback path or edge case remains unread/unproven -> subtract 2.',
    ...
    ' - An `Evidence:` line mixes a valid anchor with parenthetical code, dotted expressions, or other prose on the same line -> subtract 1.',
    ...
    ' Treat this as a delta review, not a full rewrite of prior objections.',
    ' If a blocker is unchanged from the previous round, mention it once in one short sentence instead of restating the full argument.',
    ' If newly fetched seams narrowed the problem, describe only the remaining unresolved delta.',
    ' Add or keep a `missingSeams` entry only when it is new, materially narrower, or more precise than before.',
...
function buildConsensusRevisionContent(promptText, discussionSoFar, consensusText, revisionNotes) {
  ...
  return [
    ...
    ' If revision notes show that only part of the fix is directly proven, rewrite `Grounded Fixes` as a scoped patch and move the unproven full-fix claim to `Assumptions / Unverified Seams`.',
    ' Keep the revised answer compact: preserve section order, collapse repetition, and keep each section to at most 5 bullets/items unless code blocks require more space.',
    ' Revise only the unresolved delta from the latest notes. Do not re-explain unchanged accepted sections or copy the same objection prose back into the answer.',
    ' If multiple revision notes describe the same blocker, collapse them into one concrete change request.',
    '\n\n# OUTPUT CONTRACT\n',
...
function buildDevilsAdvocateContent(originalPrompt, enhancedPrompt, consensusResult, discussionSoFar) {
  return [
    ...
    `\n${buildSameLanguageInstruction({ jsonMode: true })}`,
    '\nLanguage compliance for free-text JSON values is mandatory.',
    '\nIf the original user request is in Russian, then every free-text JSON string value must also be in Russian.',
    '\nDo not write English summaries, issue descriptions, suggestions, edge cases, or explanatory sentences unless the user explicitly requested English.',
    '\nOnly JSON keys, enum values, exact code identifiers, API names, and other exact technical literals may remain in English.',
    '\nIf you need a technical term, keep it short and embed it inside otherwise same-language text instead of switching the whole sentence to English.',
    '\n\nYou MUST:',
    ...
    '\n6a. **Check anchor purity** - Flag any `Evidence:` line that mixes a real file anchor with parenthetical code, dotted expressions, or explanatory prose on the same line.',
